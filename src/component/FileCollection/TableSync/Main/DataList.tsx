	...
	public render() {
		...
		const columns = [
			{
				...
				key: 'syncName',
				render: (value, row) => (
					<Button type='link' onClick={this.showDetails.bind(this,row)}>{value}</Button>
				)
			},
			{
				title: '数据名称',
				...
				render: (row) => {
					return (
						<div style={{display: 'flex'}}>
							{
								row.status === 1 ? <Button type='link' size='small' style={{color: 'rgb(244, 171, 55)'}}><Icon type="undo" className={'bigSize'} /></Button> : row.status === 2 ?
								<Button type='link' size='small' style={{color: 'rgb(245, 108, 134)'}} ><Icon type="stop" className={'bigSize'}/></Button> :
								row.status === 3 || row.status === 4 || row.status === 5 ?
								<Button type='link' size='small' style={{color: 'rgb(39, 202, 142)'}} ><Icon type="play-circle" className={'bigSize'} /></Button> : ''
							}
							{
								row.status !== 2 ? <Button size='small' type='link' style={{color: '#4887ED'}}><Icon type="edit" className={'bigSize'}/></Button> : ''
							}
						</div>
					)
				}
		...
		return (
			<div className={style.tableWrap}>
				...
				<Modal visible={this.state.detailVisible} title='库表同步详情' footer={null}
